...
def process_image(method):
    ...
    if method == 'brightness_correction':
        ...
        show_image(corrected_img, 355, 120, 'Perbaikan Metode 1')
    elif method == 'grayscale':
        corrected_img = grayscale(original_img)
        show_image(corrected_img, 660, 120, 'Perbaikan Metode 2')

# fungsi untuk menampilkan informasi pembuat program
def show_creator():
    creator_label = tk.Label(root, text='Nama : Muh.Bintang Adiartha   NIM : F55121032   Kelas : A')
...
def open_image():
    ...
    if file_path:
        ...
        show_image(original_img, 105, 120, 'Gambar Original')
...
brightness_button.place( x=487,y=400)
...
smoothing_button.place(x=307,y=400)
